            ...
            <form action="{{ route('todos.update', $todo) }}" method="POST" class="space-y-6">
                ...
                <div class="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100 mb-2">
                    <input type="checkbox" id="is_done" name="is_done"
                        class="w-5 h-5 text-indigo-600 rounded focus:ring-indigo-500 border-gray-300" {{ $todo->is_done ? 'checked' : '' }}>
                ...
                <div class="space-y-1">
                    ...
                    <textarea id="description" name="description" rows="4"
                        class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all placeholder:text-slate-400 resize-none">{{ old('description', $todo->description) }}</textarea>
                    @error('description')
                        <p class="text-xs font-semibold text-rose-500">{{ $message }}</p>
                    @enderror
                </div>
